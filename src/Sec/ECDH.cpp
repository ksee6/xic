/**
 * @file ECDH.cpp
 * @brief High-performance standalone implementation of Curve25519, X25519, and XEdDSA.
 */

#include "../../include/Sec/ECDH.hpp"
...
#define WIPE_CTX(c) do { volatile u8 *_p = (volatile u8*)(c); for (usz _i = 0; _i < sizeof(*(c)); _i++) _p[_i] = 0; } while (0)
...
static inline void load32_le_buf(u32 *dst, const u8 *src, usz n) {
  for (usz i = 0; i < n; ++i) dst[i] = load32_le(src + i * 4);
}
static inline void store32_le_buf(u8 *dst, const u32 *src, usz n) {
  for (usz i = 0; i < n; ++i) store32_le(dst + i * 4, src[i]);
}
...
static inline int crypto_verify32(const u8 a[32], const u8 b[32]) {
  u32 diff = 0;
  for (int i = 0; i < 32; ++i) diff |= (u32)(a[i] ^ b[i]);
  return (1 & ((diff - 1) >> 8)) - 1;
}

static inline void crypto_wipe(void *secret, usz size) {
  volatile u8 *p = (volatile u8*)secret;
  while (size--) *p++ = 0;
}
...
static void negate_scalar_mod_L(u8 a_out[32], const u8 a[32]) {
  u32 carry = 0;
  for (int i = 0; i < 32; i++) {
...
String sharedKey(const String &privateKey, const String &publicKey) {
  ...
  String shared = makeZeros(32);
  const u8 *sk = reinterpret_cast<const u8 *>(privateKey.data());
  const u8 *pk = reinterpret_cast<const u8 *>(publicKey.data());
  u8 *out = reinterpret_cast<u8 *>(shared.data());

  crypto_x25519(out, sk, pk);
  return shared;
}

String sharedKey(const KeyPair &ourKeyPair, const String &theirPublicKey) {
  return sharedKey(ourKeyPair.secretKey, theirPublicKey);
}
...
String publicKey(const String &privateKey) {
  ...
  String pub = makeZeros(32);
  const u8 *sk = reinterpret_cast<const u8 *>(privateKey.data());
  u8 *out = reinterpret_cast<u8 *>(pub.data());
...
KeyPair generateKeyPair() {
  String secret = makeZeros(32);
  u8 *raw = reinterpret_cast<u8 *>(secret.data());
  Xi::randomFill(raw, 32);
...
String signX(const String &privateKey, const String &text) {
  ...
  u8 pub_x[32];
  const u8 *privData = reinterpret_cast<const u8 *>(privateKey.data());
  crypto_x25519_public_key(pub_x, privData);
  crypto_x25519_to_eddsa(A, pub_x);

  crypto_eddsa_trim_scalar(a, privData);

  u8 a_padded[64];
  for (int i = 0; i < 32; ++i) a_padded[i] = a[i];
  for (int i = 32; i < 64; ++i) a_padded[i] = 0;
  ...
  String secret = hash(privateKey, 64);
  const u8 *prefix = reinterpret_cast<const u8 *>(secret.data()) + 32;

  Blake2bCtx ctx;
  blake2bInit(&ctx, 64);
  blake2bUpdate(&ctx, prefix, 32);
  blake2bUpdate(&ctx, text.data(), text.size());
  u8 hash_out[64];
  blake2bFinal(&ctx, hash_out);
  ...
  blake2bInit(&ctx, 64);
  blake2bUpdate(&ctx, R, 32);
  blake2bUpdate(&ctx, A, 32);
  blake2bUpdate(&ctx, text.data(), text.size());
  blake2bFinal(&ctx, hash_out);
  ...
  String signature = makeZeros(64);
  u8 *sigData = reinterpret_cast<u8 *>(signature.data());
  for (int i = 0; i < 32; i++) {
    sigData[i] = R[i];
    sigData[i + 32] = S[i];
  }
...
bool verifyX(const String &publicKey, const String &text, const String &signature) {
  ...
  crypto_x25519_to_eddsa(A, pubData);

  A[31] &= 0x7F;

  Blake2bCtx ctx;
  blake2bInit(&ctx, 64);
  blake2bUpdate(&ctx, signature.data(), 32);
  blake2bUpdate(&ctx, A, 32);
  blake2bUpdate(&ctx, text.data(), text.size());
  u8 hash_out[64];
  blake2bFinal(&ctx, hash_out);
  ...
  return (crypto_eddsa_check_equation(sigData, A, h) == 0);
}
