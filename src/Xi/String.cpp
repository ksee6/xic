/**
 * @file String.cpp
 * @brief Implementation of the Xi String class and its utility methods.
...
void String::concat(const String &other) {
  pushEach(other.data(), other.size());
}
...
long long String::indexOf(const String &needle, usz start) const {
...
bool String::includes(const String &needle, usz start) const {
  return find(needle, start) != -1;
}

bool String::startsWith(const String &prefix) const {
  usz pLen = prefix.size();
  if (pLen > size())
    return false;
  const u8 *d = data();
  const u8 *p = prefix.data();
...
bool String::endsWith(const String &suffix) const {
  usz sLen = suffix.size();
  if (sLen > size())
    return false;
  const u8 *d = data();
  const u8 *s = suffix.data();
  usz start = size() - sLen;
...
long long String::find(const String &needle, usz start) const {
  usz nLen = needle.size();
  usz hLen = size();
  if (nLen == 0)
    return (long long)start;
  if (hLen < nLen || start > hLen - nLen)
    return -1;
  const u8 *h = data();
  const u8 *n = needle.data();
...
String String::replace(const String &tgt, const String &rep) const {
...
bool String::constantTimeEquals(const String &b, int length) const {
  const u8 *ad = data();
  const u8 *bd = b.data();
...
void String::randomFill(usz len) {
  if (len == 0 || size() < len)
    len = size();
  u8 *raw = data();
  if (raw && len > 0)
    Xi::randomFill(raw, len);
}
