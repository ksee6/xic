 ...
 *        All symbols live in Math:: namespace.
 */
...
// --- Math::Stat explicit specializations for Array<f32> ---

template <> f32 sum<f32>(const Xi::Array<f32> &a) {
  f32 s = 0;
  usz n = a.size();
  const f32 *d = a.data();
...
template <> f32 mean<f32>(const Xi::Array<f32> &a) {
  usz n = a.size();
  return (n == 0) ? 0 : sum(a) / (f32)n;
}

template <> f32 var<f32>(const Xi::Array<f32> &a) {
  usz n = a.size();
  if (n == 0) return 0;
  f32 m = mean(a);
  f32 v = 0;
...
template <> f32 std<f32>(const Xi::Array<f32> &a) { return sqrt(var(a)); }
