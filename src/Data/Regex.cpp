#include "../../include/Data/Regex.hpp"

namespace Data {

void Regex::resetDFA() const {
...
void Regex::addEpsilon(int pc, Array<int> &set, Array<int> &visited) const {
...
void Regex::addConsuming(int pc, Array<int> &set, Array<int> &visited) const {
  if (pc < 0 || pc >= (int)inst.size() || visited.find(pc) != -1)
    return;
  visited.push(pc);
  Op op = inst[(usz)pc].op;
...
bool Regex::isWord(char c) const {
  return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') ||
         (c >= '0' && c <= '9') || c == '_';
}

bool Regex::checkClass(const Inst &ins, u8 c) const {
...
int Regex::getNextDFAState(int stateId, u8 c) const {
...
void Regex::emit(Array<Inst> &p, Op op, int x, int y) {
  Inst i;
  i.op = op;
  i.x = x;
  i.y = y;
...
Regex::Regex(const String &p) { compile(p); }

Array<Regex::Inst> Regex::compileSub(const String &p, int &pos) {
  Array<Inst> s;
...
void Regex::addClassRange(Inst &ci, char start, char end) {
  for (char rc = start; rc <= end; rc++)
    ci.chars += rc;
}
void Regex::addClassEscape(Inst &ci, char nc) {
  if (nc == 'd')
...
void Regex::compileCore(const String &p, int &pos, Array<Inst> &prog, int depth,
                        bool localIC) {
  ...
  while (pos < (int)p.size() && p[pos] != ')' && p[pos] != '|') {
    ...
    if (c == '^')
      ...
      if (nc == 'w' || nc == 'd' || nc == 's' || nc == 'b') {
        ...
        emit(prog, localIC || globalIgnoreCase ? Op::CharIC : Op::Char, (int)nc);
      }
    } else if (c == '[') {
      Inst ci;
      ci.op = Op::Class;
...
void Regex::compile(const String &p) {
  code = p;
...
bool Regex::runVM(const String &in, int start, const Array<Inst> &prog,
                  bool rev) const {
  struct ThreadV {
    int pc;
  };
...
Array<RegexMatch> Regex::matchAll(const String &input, int maxMatches,
                                  u64 limitUs) const {
...
void Regex::DFAState::sort() {
  for (usz i = 0; i < pcs.size(); i++)
    for (usz j = i + 1; j < pcs.size(); j++)
      if (pcs[i] > pcs[j]) {
        int tmp = pcs[i];
        pcs[i] = pcs[j];
        pcs[j] = tmp;
...
bool Regex::DFAState::operator==(const DFAState &o) const {
...
Array<String> String::split(const Data::Regex &reg) const {
  Array<String> r;
...
String String::replace(const Data::Regex &reg, const String &rep) const {
  String r;
  auto m = reg.matchAll(*this);
  long long e = 0;
  for (int i = 0; i < (int)m.size(); ++i) {
